...
exports.MongoDbUrlManager = void 0;
...
class MongoDbUrlManager {
    ...
    async getNextUrl() {
        ...
        try {
            ...
            if (!this.robotsCache[targetDomain]) {
                this.robotsCache[targetDomain] = await (0, urlUtils_1.parseRobotsTxt)(targetDomain);
            }
            // 도메인에서 URL 가져오기
            result = await this.getUrlForDomain(targetDomain);
            const runtime = Date.now() - startTime;
            logger_1.defaultLogger.eventInfo('get_next_url', {
            ...
            const runtime = Date.now() - startTime;
            logger_1.defaultLogger.error(`URL 가져오기 중 오류:`, error);
            logger_1.defaultLogger.eventInfo('get_next_url', {
                url: 'error',
    ...
    async initAvailableDomains() {
        try {
            ...
            const domains = await visitResult_1.VisitResultModel.find({}, { domain: 1, _id: 0 });
            const findRuntime = Date.now() - findStartTime;
            logger_1.defaultLogger.eventInfo('find_domain', { runtime: findRuntime });
            if (domains && domains.length > 0) {
                // 중복 없는 도메인 목록 생성
                this.availableDomains = domains.map((doc) => ({ domain: doc.domain }));
                logger_1.defaultLogger.debug(`${this.availableDomains.length}개의 도메인을 불러왔습니다.`);
                // 도메인 목록 로깅 (최대 5개)
                if (this.availableDomains.length > 0) {
                    const domainSample = this.availableDomains.slice(0, 5).map(d => d.domain);
                    logger_1.defaultLogger.debug(`도메인 샘플: ${domainSample.join(', ')}${this.availableDomains.length > 5 ? ` 외 ${this.availableDomains.length - 5}개` : ''}`);
                }
            }
            else {
                ...
                logger_1.defaultLogger.debug(`도메인이 없어 시작 도메인 ${startDomain}으로 초기화합니다.`);
            }
        }
        catch (error) {
            logger_1.defaultLogger.debug('도메인 목록 로드 중 오류:', error);
            // 오류 시 기본 시작 URL의 도메인 사용
            const startDomain = this.specificDomain || '';
            this.availableDomains = [{ domain: startDomain }];
            logger_1.defaultLogger.debug(`오류로 인해 시작 도메인 ${startDomain}으로 초기화합니다.`);
        }
    }
    /**
        ...
        switch (this.strategy) {
            ...
            case 'random':
                ...
                if (!this.availableDomains || this.availableDomains.length === 0) {
                    ...
                    logger_1.defaultLogger.debug(`랜덤 도메인 선택: ${targetDomain} (인덱스: ${randomIndex}/${this.availableDomains.length})`);
                }
                break;
            case 'sequential':
            default:
                ...
                if (!this.availableDomains || this.availableDomains.length === 0 || this.currentDomainIndex === undefined) {
                    ...
                    logger_1.defaultLogger.debug(`순차적 도메인 선택: ${targetDomain} (인덱스: ${this.currentDomainIndex}/${this.availableDomains.length})`);
                    // 다음 도메인으로 인덱스 이동
    ...
    async getUrlForDomain(targetDomain) {
        try {
            // 도메인 문서 가져오기
            const findStartTime = Date.now();
            const domainDoc = await visitResult_1.VisitResultModel.findOne({ domain: targetDomain });
            const findRuntime = Date.now() - findStartTime;
            logger_1.defaultLogger.eventInfo('find_domain_document', { domain: targetDomain, runtime: findRuntime });
            ...
            logger_1.defaultLogger.eventInfo('filter_urls', {
                domain: targetDomain,
            ...
            if (allowedUnvisitedUrls.length > 0) {
                ...
                logger_1.defaultLogger.debug(`도메인 ${targetDomain}에서 방문할 URL을 찾았습니다: ${unvisitedUrl.url}`);
                this._recursionCount = 0; // 재귀 카운터 초기화
                return { url: unvisitedUrl.url, domain: targetDomain };
            }
            // 방문할 URL이 없는 경우
            else {
                logger_1.defaultLogger.debug(`도메인 ${targetDomain}에 방문 가능한 URL이 없습니다.`);
                return this.tryNextDomain();
            }
        }
        catch (error) {
            logger_1.defaultLogger.error(`도메인 ${targetDomain}에서 URL 가져오기 실패:`, error);
            return this.tryNextDomain();
        }
    }
    /**
    ...
    async filterAllowedUrls(urls, targetDomain) {
        ...
        logger_1.defaultLogger.debug(`도메인 ${targetDomain}에서 방문하지 않은 URL ${unvisitedUrls.length}개 발견`);
        // 빈 URL 리스트인 경우 조기 반환
        ...
        for (let i = 0; i < unvisitedUrls.length; i += batchSize) {
            ...
            const batchPromises = batch.map(item => (0, urlUtils_1.isUrlAllowedWithRobots)(item.url, [targetDomain], this.robotsCache)
                .then(isAllowed => isAllowed ? item : null));
            const batchResults = await Promise.all(batchPromises);
            results.push(...batchResults.filter(item => item !== null));
            // 충분한 URL을 찾으면 조기 반환 (최적화)
            if (results.length >= 5) {
                logger_1.defaultLogger.debug(`허용된 URL을 충분히 찾았습니다. 나머지 URL 필터링 생략.`);
                break;
            }
        }
        return results;
    ...
    async tryNextDomain() {
        ...
        if (!this.availableDomains ||
            ...
            logger_1.defaultLogger.debug('모든 도메인에 방문할 URL이 없습니다.');
            this._recursionCount = 0;
            return null;
        }
        logger_1.defaultLogger.debug(`다른 도메인 시도 중... (${this._recursionCount}/${this.availableDomains?.length || 0})`);
    ...
    async handleDomainError() {
        logger_1.defaultLogger.debug('다른 도메인에서 URL 가져오기 시도...');
        if (!this._errorCount)
            this._errorCount = 0;
        this._errorCount++;
        if (this._errorCount > 3) {
            logger_1.defaultLogger.debug('너무 많은 오류가 발생했습니다.');
            this._errorCount = 0;
            return null;
        }
        return this.getNextUrl();
...
exports.MongoDbUrlManager = MongoDbUrlManager;
//# sourceMappingURL=MongoDbUrlManager.js.map