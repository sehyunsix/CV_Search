...
describe('GeminiParser', () => {
    ...
    let mockRecruitInfoRepository;
    let mockCacheRecruitInfoRepository;
    let mockRedisUrlManager;
    ...
    beforeEach(() => {
        ...
        parser = new GeminiParser_1.GeminiParser(options);
    });
    // 기본 테스트 - 모킹 없이 기본 설정 테스트
    describe('Basic Functionality', () => {
        // 파서 인스턴스가 생성되는지 테스트
        test('should create parser instance successfully', () => {
            expect(parser).toBeInstanceOf(GeminiParser_1.GeminiParser);
        });
    ...
    describe('Parsing and API Functionality', () => {
        ...
        test('should rotate API key on rate limit error', async () => {
            ...
            parser['_initializeClient'] = jest.fn().mockImplementation(() => { parser['genAI'] = new generative_ai_1.GoogleGenerativeAI('temp'); });
            // 실행
        ...
        test('should retry API call on rate limit error', async () => {
            ...
            parser['currentKeyIndex'] = 0;
            parser['apiKey'] = 'key1';
            parser['genAI'] = {}; // API 클라이언트가 초기화되어 있다고 가정
        ...
        test('should update URL status after parsing', async () => {
            ...
            // 검증
            expect(setURLStatusSpy).toHaveBeenCalledWith('https://example.com/jobs/developer', "hasRecruitInfo" /* URLSTAUS.HAS_RECRUITINFO */);
        });
