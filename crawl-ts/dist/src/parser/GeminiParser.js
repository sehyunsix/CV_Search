...
function geminiRegionTextPrompt(content) {
    return `
...
function geminiRecruitInfoPrompt(content) {
    return `
...
class GeminiParser {
    ...
    *cycleKeyGenerator() {
    ...
    async ParseRegionText(rawContent, retryNumber, retryDelay = 1000) {
        for (let attempt = 1; attempt <= retryNumber; attempt++) {
            try {
                const model = new generative_ai_1.GoogleGenerativeAI(this.apiKeyGenerator.next().value).getGenerativeModel({
                ...
                const result = await model.generateContent(geminiRegionTextPrompt(rawContent))
                    ...
                    .catch((error) => {
                    logger_1.defaultLogger.error(`Gemini API에서 텍스트 응답을 받지 못했습니다.${attempt}/${retryNumber}`);
                    ...
                    .then((responseText) => {
                    if (!responseText) {
                        logger_1.defaultLogger.error(`Gemini API에서 빈 응답을 받았습니다.${attempt}/${retryNumber}`);
                    ...
                    .catch((error) => {
                    logger_1.defaultLogger.error(`텍스트 응답에에서 json 파싱을을 실패했습니다. ${attempt}/${retryNumber}`);
                    if (retryNumber === attempt) {
                        throw error;
                    }
                });
                ...
                logger_1.defaultLogger.error(`재시도 횟수 ${attempt}/${retryNumber} 증 에러 발생`);
                if (retryNumber === attempt) {
    ...
    parseDateOrNull(dateStr) {
        try {
            ...
            logger_1.defaultLogger.error(`날짜 파싱 중 오류 발생: ${error}`);
            return undefined;
        }
    }
    /**x
    ...
    */
    async parseRawContentRetry(rawContent, retryNumber, retryDelay = 1000) {
        for (let attempt = 1; attempt <= retryNumber; attempt++) {
            try {
                const model = new generative_ai_1.GoogleGenerativeAI(this.apiKeyGenerator.next().value).getGenerativeModel({
                    model: this.modelName,
                ...
                // API 호출로 채용 정보 파싱
                logger_1.defaultLogger.debug('[GeminiParser][parseRawContentRetry] Gemini API 요청 시작...');
                const result = await model.generateContent(geminiRecruitInfoPrompt(rawContent.text))
                    .then((result) => result.response?.text())
                    .catch((error) => {
                    logger_1.defaultLogger.error(`[GeminiParser][parseRawContentRetry] Gemini API에서 텍스트 응답을 받지 못했습니다.${attempt}/${retryNumber}`);
                    if (retryNumber === attempt) {
                        throw error;
                    }
                })
                    .then((responseText) => {
                    if (!responseText) {
                        logger_1.defaultLogger.error(`[GeminiParser][parseRawContentRetry] Gemini API에서 빈 응답을 받았습니다.${attempt}/${retryNumber}`);
                        throw new ParseError('Gemini API에서 빈 응답을 받았습니다.');
                    }
                    ...
                    if (!data.require_experience) {
                        data.require_experience = "경력무관";
                    }
                    if (!data.title) {
                        data.title = rawContent.title;
                    }
                    if (!data.region_id) {
                        data.region_id = [];
                    }
                    if (rawContent.text && data.region_id) {
                        data.region_id = data.region_id.map((regionCd) => (0, Transform_1.cd2RegionId)(regionCd)).filter((regionId) => regionId !== undefined);
                    }
                    if (data.region_text) {
                        data.region_id.concat((0, Transform_1.regionText2RegionIds)(data.region_text || ''));
                    }
                    if (data.region_id.length === 0) {
                        data.region_id = [Transform_1.OTHER_REGION_ID];
                    }
                    return data;
                })
                    .catch((error) => {
     ...
     * @param rawContent 원본 콘텐츠
     */
    makeDbRecruitInfo(botRecruitInfo, rawContent, favicon) {
        const now = new Date();
        return {
            ...botRecruitInfo,
            ...rawContent,
