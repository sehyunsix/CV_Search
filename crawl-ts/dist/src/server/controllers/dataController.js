...
const getResults = async (req, res, next) => {
    try {
        ...
        const results = await VisitResult_1.VisitResultModel.aggregate(aggregationPipeline);
        // 결과 개수 계산
        const countResults = await VisitResult_1.VisitResultModel.aggregate(countPipeline);
        const totalCount = countResults.length > 0 ? countResults[0].totalCount : 0;
        logger_1.defaultLogger.info(`검색 완료: 키워드 [${parsedKeywords.join(', ')}], ${totalCount}개 결과 중 ${results.length}개 반환`);
        // Format response
        const response = {
            success: true,
        ...
        logger_1.defaultLogger.error('검색 오류:', error);
        next(error);
    }
};
/**
...
const getUrlDetails = async (req, res, next) => {
    try {
        ...
        logger_1.defaultLogger.info(`URL 상세 정보 요청: ${decodedUrl}`);
        // URL 정보 조회
        const result = await VisitResult_1.VisitResultModel.findOne({ 'suburl_list.url': decodedUrl }, { 'suburl_list.$': 1, domain: 1 });
        if (!result || !result.suburl_list || result.suburl_list.length === 0) {
            logger_1.defaultLogger.warn(`URL을 찾을 수 없음: ${decodedUrl}`);
            res.status(404).json({ success: false, message: 'URL not found' });
            return;
        }
        const urlData = result.suburl_list[0];
        // 해당 URL에 대한 SubUrl 인스턴스 생성 (메서드 사용을 위해)
        const subUrl = new VisitResult_1.SubUrl(urlData);
        // 응답 형식 지정
        const response = {
            success: true,
            result: {
        ...
        logger_1.defaultLogger.info(`URL 상세 정보 반환: ${decodedUrl}`);
        res.status(200).json(response);
    }
    catch (error) {
        logger_1.defaultLogger.error('URL 상세 정보 오류:', error);
        next(error);
    }
};
/**
...
const getDomainStats = async (req, res, next) => {
    try {
        ...
        const parsedPage = parseInt(page) || 1;
        const skip = (parsedPage - 1) * parsedLimit;
        logger_1.defaultLogger.info(`도메인 통계 요청: 페이지 ${parsedPage}, 항목 수 ${parsedLimit}`);
        // 도메인별 통계 집계 파이프라인
        const aggregationPipeline = [
            {
                $project: {
        ...
        const totalCount = await VisitResult_1.VisitResultModel.countDocuments({});
        // 도메인 통계 조회
        const domains = await VisitResult_1.VisitResultModel.aggregate(aggregationPipeline);
        // 전체 통계 요약
        const summary = await VisitResult_1.VisitResultModel.aggregate([
            {
                $group: {
                    _id: null,
                    totalDomains: { $sum: 1 },
        ...
        logger_1.defaultLogger.info(`도메인 통계 조회 완료: ${domains.length}개 도메인 정보 반환`);
        // 응답 형식 지정
        const response = {
            success: true,
        ...
        logger_1.defaultLogger.error('도메인 통계 오류:', error);
        next(error);
    }
};
/**
 ...
 */
const resetVisitedStatus = async (req, res, next) => {
    try {
        // MongoDB 연결 확인
        const { domain, onlyFailed } = req.query;
        logger_1.defaultLogger.info(`방문 상태 초기화 요청: 도메인=${domain || '전체'}, 실패만=${onlyFailed ? '예' : '아니오'}`);
        let result;
        if (onlyFailed === 'true') {
            // 실패한 URL만 초기화
            const filter = typeof domain === 'string' ? { domain } : {};
            const updateResult = await VisitResult_1.VisitResultModel.updateMany(filter, { $set: { 'suburl_list.$[elem].visited': false, 'suburl_list.$[elem].updated_at': new Date() } }, {
        ...
        logger_1.defaultLogger.info(`방문 상태 초기화 완료: ${result.message}`);
        res.status(200).json(result);
    }
    catch (error) {
        logger_1.defaultLogger.error('방문 상태 초기화 오류:', error);
        next(error);
    }
};
/**
...
const getFavicon = async (req, res, next) => {
    try {
        ...
        logger_1.defaultLogger.info(`Favicon 요청: ${domain}`);
        // 도메인에 대한 문서 조회
        const result = await VisitResult_1.VisitResultModel.findOne({ domain }, { favicon: 1 });
        if (!result || !result.favicon) {
            logger_1.defaultLogger.warn(`도메인의 favicon을 찾을 수 없음: ${domain}`);
        ...
        logger_1.defaultLogger.debug(`Favicon 성공적으로 반환: ${domain}`);
    }
    catch (error) {
        logger_1.defaultLogger.error('Favicon 서비스 오류:', error);
        next(error);
    }
};
/**
 ...
 */
const getAllFavicons = async (req, res, next) => {
    try {
        logger_1.defaultLogger.info('모든 favicon 목록 요청');
        // favicon 필드가 존재하는 모든 도메인 조회
        const results = await VisitResult_1.VisitResultModel.find({ favicon: { $exists: true, $ne: null } }, { domain: 1, _id: 0 }).sort({ domain: 1 });
        if (!results || results.length === 0) {
            logger_1.defaultLogger.warn('저장된 favicon이 없습니다.');
            res.status(200).json({ success: true, domains: [] });
            return;
        }
        // 도메인 목록 추출
        const domains = results.map(result => result.domain);
        logger_1.defaultLogger.info(`총 ${domains.length}개의 favicon 도메인을 찾았습니다.`);
        res.status(200).json({
            success: true,
        ...
        logger_1.defaultLogger.error('Favicon 목록 조회 오류:', error);
        next(error);
    }
};
exports.default = {
