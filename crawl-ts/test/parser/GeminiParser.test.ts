...
describe("Gemini Parser Test", () => {
  ...
  test('parseRawContent는 rawContent를 파싱했을때,is_recruit_info가 null 없이 반환해야한다.', async () => {
    const botRespone = await geminiParser.loadMongoRawContent(1).then(
      (results) => geminiParser.parseRawContentRetry(results[0], 4)
    )
  ...
  test('parseRawContent는 rawContent를 파싱할때, 비동기 처리가 가능해야한다.', async () => {
    ...
    const botResponses = await Promise.all(
      rawContents.map(rawContent => geminiParser.parseRawContentRetry(rawContent, 4))
    );
