 ...
 */
  ...
  */
  ...
  async deleteRecruitInfoById(id: number, token: string): Promise<void> {
    try {
      ...
      if (response.status === 200) {
        logger.info(`✅ Job ${id} 삭제 성공`);
      } else {
        logger.warn(`⚠️ Job ${id} 삭제 응답 코드: ${response.status}`);
      }
    } catch (error) {
      logger.error(`❌ Job ${id} 삭제 실패`, error);
      throw error;
    }
  }
   ...
   * 채용 정보 유효성 타입 업데이트
   * @param id 채용 정보 ID
   * @param validType 유효성 타입 (예: ACTIVE, EXPIRED 등)
   * @returns 업데이트된 채용 정보 객체 또는 null (업데이트 실패 시)
   */
  ...
  async vectorizeJob(): Promise<void> {
    try {
      const response = await axios.get(`${process.env.SPRING_API_DOMAIN}/job-update/start`);
      if (response.status === 200) {
        logger.info(`✅ 벡터화 작업 성공`);
      } else {
        logger.warn(`⚠️ 벡터화 작업 응답 코드: ${response.status}`);
      }
    }catch (error) {
      logger.error(`❌ 벡터화 작업 실패`, error);
      throw error;
    }
