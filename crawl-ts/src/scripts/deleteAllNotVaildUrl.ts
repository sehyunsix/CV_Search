...
if (require.main === module) {
  (async () => {
    ...
    await MysqlRecruitInfoSequelize.findAll({'attributes': ['id', 'url'], 'where': { job_valid_type: VALID_TYPE.DEFAULT }, raw: true })
      .then(async (datas) => {
        const deleteCount = datas.length;
        logger.debug(`삭제할 URL 갯수: ${deleteCount}`);
        for (const data of datas) {
           await mysqlRecruitInfoRepository.deleteRecruitInfoByIdValidType(data.id,VALID_TYPE.DEFAULT, token)
             .then(() => {
