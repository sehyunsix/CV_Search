...
function logEnvironmentOverrides() {
    ...
    if (process.env.DATA_DIR)
        overrides.push("DATA_DIR: ".concat(process.env.DATA_DIR));
    if (process.env.MAX_URLS)
        overrides.push("MAX_URLS: ".concat(process.env.MAX_URLS));
    if (process.env.DELAY)
        overrides.push("DELAY: ".concat(process.env.DELAY, "ms"));
    if (process.env.HEADLESS)
        overrides.push("HEADLESS: ".concat(process.env.HEADLESS));
    if (overrides.length > 0) {
        console.log('환경 변수에 의해 재정의된 설정:');
        overrides.forEach(function (override) { return console.log("- ".concat(override)); });
    }
}
// 설정 초기화 함수
    ...
    directories.forEach(function (dir) {
        if (!fs.existsSync(dir)) {
            try {
                ...
                console.log("\uB514\uB809\uD1A0\uB9AC \uC0DD\uC131\uB428: ".concat(dir));
            }
            catch (error) {
                console.error("\uB514\uB809\uD1A0\uB9AC \uC0DD\uC131 \uC2E4\uD328: ".concat(dir), error);
            }
        }
    });
    // 환경 변수에 의해 재정의된 설정 로그
    logEnvironmentOverrides();
    console.log("\uD06C\uB864\uB9C1 \uC124\uC815 \uCD08\uAE30\uD654 \uC644\uB8CC. \uB370\uC774\uD130 \uB514\uB809\uD1A0\uB9AC: ".concat(PATHS.DATA_DIR));
}
